...
class TtMoE(LightweightModule):
    ...
    def forward(self, x):
        ...
        for t0 in range(0, T, _DENSE_TMAX):
            ...
            tc = t1 - t0
            with prof.phase(self.mesh_device, "moe.repeat"):
                xe = ttnn.repeat(
                    ttnn.reshape(ttnn.slice(x2, [t0, 0], [t1, hidden]), [1, tc, hidden]), ttnn.Shape([E, 1, 1])
                )  # [E, tc, hidden]
            with prof.phase(self.mesh_device, "moe.gate_up_mm"):
                hgu = ttnn.matmul(
                    xe,
                    gate_up_w,
            ...
            with prof.phase(self.mesh_device, "moe.swiglu"):
            ...
            with prof.phase(self.mesh_device, "moe.down_mm"):
                ye = ttnn.matmul(
                    h,
                    down_w,
            ...
            with prof.phase(self.mesh_device, "moe.reduce"):
