...
| `QWEN36_VERIFY_BATCH_SDPA` | 1 | speculative VERIFY emits all K rows from ONE `paged_scaled_dot_product_attention_decode` over a `zeros([K,1])` page table (the paged op takes its batch from the page table, not the KV cache, and a flat cache is already a 1-block paged cache) instead of looping K flat `sdpa_decode` calls. No extra memory, exactly neutral at K=1, and PCC 1.0 with bit-identical KV vs the loop. Paired A/B on the verify trace, 40L: **−1.9% at gamma=2 / −4.8% at K=16** at short context, **−2.7% / −6.7%** at ~2048 — the marginal cost per speculative token drops 6.84 → 6.26 ms, so it compounds with K. `0` reverts to the row loop (the reference). See MTP.md |
