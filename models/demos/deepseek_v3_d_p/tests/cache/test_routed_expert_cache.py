...
def test_routed_expert_weights_cold_warm_cache(mesh_device, device_params):
    ...
    def written_rows(output_4d):
        ...
        The unified op allocates `expert_outputs` with ttnn::empty and `insert`
        writes each expert's ceil_tile(count) rows at its region offset; the rest
        of the dispatch-shaped buffer (zero-count expert regions and the tail of
        the capacity-factor-padded buffer — typically the large majority of rows)
        is left uninitialized. Comparing the full buffer would diff harmless
        uninitialized padding (different DRAM garbage per allocation), so we
        compare only the written rows — same per-expert slicing as the op test.
        """
        rows = []
