...
inline float prelu(float x, float alpha = 0.25f) {
    return x >= 0.0f ? x : alpha * x;
}
...
inline float relu6(float x) {
    return std::max(0.0f, std::min(6.0f, x));
}

inline float celu(float x, float alpha = 1.0f) {
    return x >= 0.0f ? x : alpha * (std::exp(x / alpha) - 1.0f);
}

inline float threshold(float x, float threshold = 0.0f, float value = 0.0f) {
...
inline float hardshrink(float x, float lambda = 0.5f) {
    if (std::abs(x) > lambda) return x;
    return 0.0f;
}

inline float tanhshrink(float x) {
    return x - std::tanh(x);
}
...
inline float exp_fn(float x) {
    return std::exp(x);
}

inline float sin_fn(float x) {
    return std::sin(x);
}

inline float cos_fn(float x) {
    return std::cos(x);
}

inline float sinh_fn(float x) {
    return std::sinh(x);
}

inline float cosh_fn(float x) {
    return std::cosh(x);
}

inline float atanh_fn(float x) {
    return std::atanh(x);
}
...
inline float compute_reference(const std::string& activation, float x) {
    ...
    if (activation == "tanh") return tanh_fn(x);
    if (activation == "relu") return relu(x);
    if (activation == "gelu") return gelu(x);
    if (activation == "swish") return swish(x);
    if (activation == "elu") return elu(x);
    if (activation == "selu") return selu(x);
    if (activation == "leaky_relu") return leaky_relu(x);
    if (activation == "prelu") return prelu(x);
    if (activation == "softplus") return softplus(x);
    if (activation == "softsign") return softsign(x);
    if (activation == "mish") return mish(x);
    if (activation == "hardtanh") return hardtanh(x);
    if (activation == "hardsigmoid") return hardsigmoid(x);
    if (activation == "hardswish") return hardswish(x);
    if (activation == "relu6") return relu6(x);
    if (activation == "celu") return celu(x);
    if (activation == "threshold") return threshold(x);
    if (activation == "softshrink") return softshrink(x);
    if (activation == "hardshrink") return hardshrink(x);
    if (activation == "tanhshrink") return tanhshrink(x);
    if (activation == "logsigmoid") return logsigmoid(x);
    if (activation == "exp") return exp_fn(x);
    if (activation == "sin") return sin_fn(x);
    if (activation == "cos") return cos_fn(x);
    if (activation == "sinh") return sinh_fn(x);
    if (activation == "cosh") return cosh_fn(x);
    if (activation == "atanh") return atanh_fn(x);
    if (activation == "erf") return erf_fn(x);
