...
public:
    ...
    static std::vector<float> load_from_file(const std::string& filename) {
        ...
        if (!file || size == 0 || size > 1000000) {  // Sanity check
            throw std::runtime_error(
                "Invalid LUT file header in: " + filename +
        ...
        if (!file) {
            throw std::runtime_error(
                "Failed to read LUT data from: " + filename +
                " (expected " + std::to_string(size) + " floats)");
        }
    ...
    static std::vector<float> load_and_validate(const std::string& filename) {
        ...
        if (lut_data.size() != ExpectedSize) {
            throw std::runtime_error(
                "LUT size mismatch in " + filename + ": " +
                "file has " + std::to_string(lut_data.size()) +
     ...
     */
    static void save_as_text(const std::vector<float>& lut_data,
