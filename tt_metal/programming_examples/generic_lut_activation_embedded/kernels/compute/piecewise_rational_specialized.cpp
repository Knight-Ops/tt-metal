...
inline void unroll_segment_rational_deferred(const std::array<float, LUT_SIZE>& lut,
                                              vFloat x_clamped, vFloat x,
                                              vFloat& numer, vFloat& denom
    ...
    if constexpr (SEG < NUM_SEGMENTS) {
        ...
        v_if (x_clamped >= lut[SEG]) {
            eval_rational_numer_denom<NUM_DEGREE, DEN_DEGREE>(
                &lut[CO + SEG * CPS], &lut[CO + SEG * CPS + NUM_COEFFS], x, numer, denom
#if defined(RATIONAL_NUM_PARITY_ODD) && defined(RATIONAL_DEN_PARITY_EVEN)
                , x2
#endif
            );
