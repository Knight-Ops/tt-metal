...
#include <chrono>
#include <sstream>
#include "../generic_lut_activation/exhaustive_bf16_generator.hpp"
...
#define COMPUTE_KERNEL_PATH OVERRIDE_KERNEL_PREFIX "generic_lut_activation_embedded/kernels/compute/" KERNEL_VARIANT ".cpp"
#else
#define COMPUTE_KERNEL_PATH OVERRIDE_KERNEL_PREFIX "generic_lut_activation_embedded/kernels/compute/piecewise_constant.cpp"
#endif

int main(int argc, char** argv) {
    if (argc < 2) {
        fmt::print(stderr, "Usage: {} --activation <name> --range-min <min> --range-max <max> [--tiles N] [--precision bf16|fp32|both]\n", argv[0]);
        fmt::print(stderr, "\nRequired arguments:\n");
        ...
        fmt::print(stderr, "  --precision bf16|fp32|both Data precision (default: fp32). 'both' loops over bf16 then fp32.\n");
        fmt::print(stderr, "  --no-dual-eval            Disable dual x-vector evaluation (enabled by default; gives 1.25x SFPU speedup)\n");
        fmt::print(stderr, "  --no-adaptive-degree      Disable per-segment adaptive degree optimization (all segments use POLY_DEGREE)\n");
        return 1;
    }
    ...
    try {
        ...
        for (uint32_t current_tiles : batch_tiles) {
            ...
            fmt::print("Processing {} tiles ({} elements total, {} bytes per element)\n",
                       current_tiles, current_tiles * elements_per_tile, element_size);
            ...
            if (use_bf16) {
                ...
                size_t unique_count = fill_buffer_with_exhaustive_bf16(
                    input_ptr, num_elements, test_min, test_max);
            ...
            if (!dump_csv_base.empty()) {
                ...
                if (csv_file.is_open()) {
                    ...
                    for (size_t i = 0; i < num_elements; ++i) {
                        ...
                        fmt::format_to(std::back_inserter(chunk), "{:.17e},{:.17e}\n",
                                       get_input_value(i), get_output_value(i));
